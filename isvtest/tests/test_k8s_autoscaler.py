...
def _managed_step_output(
    *,
    provider: object = "managed",
    node_pool: object = "system",
    enabled: object = True,
    min_nodes: object = 1,
    max_nodes: object = 3,
) -> dict[str, object]:
    """Build a setup step output carrying provider-managed autoscaler evidence."""
    return {
        "success": True,
        "kubernetes": {
            "autoscaler": {
...
def test_cluster_autoscaler_passes_on_provider_managed_evidence_without_kubectl() -> None:
    """Verify verified provider-managed evidence passes without probing an in-cluster Deployment."""
    check = K8sClusterAutoscalerCheck(config={"require_autoscaler": True, "step_output": _managed_step_output()})
    with (
        patch("isvtest.validations.k8s_autoscaler.get_kubectl_base_shell", return_value="kubectl"),
    ...
    assert check.passed, check.message
    assert "Provider-managed Cluster Autoscaler verified" in check.message
    assert "node_pool=system" in check.message
    assert "min_nodes=1" in check.message
    assert "max_nodes=3" in check.message
    mock_run.assert_not_called()


def test_cluster_autoscaler_accepts_string_typed_managed_bounds() -> None:
    """Verify managed evidence with string-typed enabled/min/max (e.g. rendered) is accepted."""
    check = K8sClusterAutoscalerCheck(
        config={
            "step_output": _managed_step_output(enabled="true", min_nodes="0", max_nodes="5"),
        }
    )
    with (
        patch("isvtest.validations.k8s_autoscaler.get_kubectl_base_shell", return_value="kubectl"),
...
def test_cluster_autoscaler_ignores_disabled_managed_evidence_and_fails_when_required() -> None:
    """Verify disabled managed evidence falls back to Deployment discovery (fail when required)."""
    check = K8sClusterAutoscalerCheck(
        config={
            "label_selectors": [],
            "namespaces": ["kube-system"],
            "require_autoscaler": True,
            "step_output": _managed_step_output(enabled=False),
        }
    )
    with (
        patch("isvtest.validations.k8s_autoscaler.get_kubectl_base_shell", return_value="kubectl"),
...
def test_cluster_autoscaler_ignores_malformed_managed_bounds() -> None:
    """Verify managed evidence with an inverted min/max bound is not accepted as proof."""
    check = K8sClusterAutoscalerCheck(
        config={
            "label_selectors": [],
            "namespaces": ["kube-system"],
            "step_output": _managed_step_output(min_nodes=5, max_nodes=3),
        }
    )
    with (
        patch("isvtest.validations.k8s_autoscaler.get_kubectl_base_shell", return_value="kubectl"),
