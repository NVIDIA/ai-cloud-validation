...
RELEASE_NOTES_SEED = textwrap.dedent("""\
    ...
    - New foo validation (#101).

    ### Fixed

    - Stop bar from leaking (#102).
...
class TestPromoteUnreleased:
    ...
    def test_promotes_unreleased_and_inserts_template(self, release_notes: Path, tmp_path: Path) -> None:
        ...
        promoted_idx = text.index("## [0.7.1] - 2026-06-01")
        assert unreleased_idx < promoted_idx, "fresh [Unreleased] must precede the promoted heading"
        promoted_block = text[promoted_idx:]
        assert "New foo validation (#101)." in promoted_block
    ...
    def test_warns_when_unreleased_is_empty(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        err = capsys.readouterr().err
        assert "[Unreleased] section was empty" in err
    ...
    def test_warns_when_no_unreleased_heading(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        err = capsys.readouterr().err
        assert "no `## [Unreleased]` heading" in err
        assert "## [0.7.1]" not in path.read_text()
