...
def forge_request(
    ...
    *,
    ...
    Args:
        org: NGC org name.
        path: API path relative to /carbide/ (e.g., "sshkey", "sshkeygroup/{id}").
        token: Bearer token.
        base_url: NICo API base URL.
...
def forge_post(
    ...
    """POST a JSON body to a NICo API path (``/carbide/`` segment)."""
...
def forge_patch(
    ...
    """PATCH a JSON body to a NICo API path (``/carbide/`` segment)."""
...
def forge_delete(org: str, path: str, token: str, *, base_url: str, timeout: int = 30) -> dict[str, Any]:
    """DELETE a NICo API resource (``/carbide/`` segment)."""
