...
.147+| Infrastructure and Data Services
...
.48+| SDN Controller
.38+| Controls software-defined networking overlays and fabric behavior to match the compute-on-demand requirements. Enables network segmentation, isolation, dynamic network configuration, and security groups.
.38+|
| [[SDN01-01]]SDN01-01
| SDN01
...
| [[SDN17-02]]SDN17-02
| SDN17
| https://github.com/NVIDIA/ai-cloud-validation/issues/598[#598]
| kubernetes, min_req, network
| tenant
| The Kubernetes half of SDN17, asserting cluster objects rather than packages on a host. Says nothing about any host IMEX daemon - the driver's two ownership modes disagree on whether one should be running.
| P0
a| https://github.com/NVIDIA/ai-cloud-validation/issues/598[#598]
| LimitedEnv
| M9
|
| IMEX (Kubernetes DRA): On a cluster offering multi-node NVLink through the resource-allocation driver, assert the compute-domain device classes are registered and every GPU node in an NVLink clique publishes compute-domain resources through the driver's per-node plugin, without tenant installation.
| approved
|
...
| [[SDN18-02]]SDN18-02
| SDN18
| https://github.com/NVIDIA/ai-cloud-validation/issues/623[#623]
| kubernetes, min_req, network, slow
| tenant
| The Kubernetes DRA reading of SDN18, asserting cluster objects rather than a host service unit. Destructive - allocates a compute domain, deletes a daemon inside it, and releases the domain afterwards, so it needs an idle cluster. Skips where the driver defers the daemon to an operator-run host service, read from the mode the driver declares rather than from the absence of a daemon.
| P1
a| https://github.com/NVIDIA/ai-cloud-validation/issues/623[#623]
| LimitedEnv
| M9
|
| IMEX (Kubernetes DRA): On a cluster where the resource-allocation driver owns the IMEX daemon, assert every node of an allocated compute domain runs a driver-started daemon and is a ready domain member, then terminate one daemon in place and assert the driver restores that node to ready membership unaided within a bounded time.
| approved
|
...
| [[SDN19-02]]SDN19-02
| SDN19
| https://github.com/NVIDIA/ai-cloud-validation/issues/624[#624]
| kubernetes, min_req, network, slow
| tenant
| The Kubernetes DRA reading of SDN19, asserting cluster objects rather than a host service unit. The departure is a shrink of the domain - withdrawing one node's channel claim - because a drain leaves the driver's DaemonSet pods running and would report a departure that never happened. What peers observed is read from a surviving daemon's own report, never from the domain's record of the departed node, so stale controller bookkeeping is not mistaken for members noticing. Destructive - removes a node from a live domain, restores it, and releases the domain afterwards, so it needs an idle cluster. Skips where the driver defers the daemon to an operator-run host service.
| P1
a| https://github.com/NVIDIA/ai-cloud-validation/issues/624[#624]
| LimitedEnv
| M9
|
| IMEX (Kubernetes DRA): Remove one node from an allocated compute domain on purpose, and assert its driver-managed daemon exits cleanly, a surviving member reports the node unavailable within a bounded time, and the domain stays operational among the nodes that remain.
| approved
|
...
| [[SDN20-02]]SDN20-02
| SDN20
| https://github.com/NVIDIA/ai-cloud-validation/issues/625[#625]
| kubernetes, min_req, network, slow
| tenant
| The Kubernetes DRA reading of SDN20, asserting cluster objects rather than a host service unit. The reboot is confirmed by the node's boot identity changing rather than by the node answering again, since a node that never went down is reachable too, and the four recovery stages are timed separately so a partial recovery names the stage that did not complete. The reboot itself is the one thing the cluster API cannot do, so the provider supplies an out-of-band command the check runs at the point in the sequence where the domain is already up. Destructive and slow - reboots a member of a live domain, so it needs an idle cluster. Skips where the driver defers the daemon to an operator-run host service, or where no out-of-band reboot mechanism is configured.
| P1
a| https://github.com/NVIDIA/ai-cloud-validation/issues/625[#625]
| LimitedEnv
| M9
|
| IMEX (Kubernetes DRA): Reboot one node of an allocated compute domain, and assert that with no intervention it re-registers, republishes its NVLink clique label, gets the domain's daemon back, and rejoins a ready compute domain for the same workload.
| approved
|
